...
void main()
{
   ...
   sum += texture2D(texture, vec2(texcoord.x - 5.0*blurSize, texcoord.y)) * 0.02;
   ...
   vec4 bl = sum*(intensity+cos(iGlobalTime)*0.2);
   vec4 nor = texture2D(texture, vec2(texcoord.x, texcoord.y));
   gl_FragColor = nor + bl;
}